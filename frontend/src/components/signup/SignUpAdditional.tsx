...
const SignUpAdditional = () => {
  ...
  const getOptionsBySizeType = () => {
    switch (selectedOption){
      case "mm": return mmOptions;
      case "EU": return euOptions;
      case "US": return usOptions;
      default: return [];
    }
  };
      ...
      <form className="gap-4" onSubmit={submitHandle}>
        <div className="gap-6 py-4">
          ...
          <InputField title="평소 신는 스니커즈 사이즈">
            <DropDown 
            ref={dropdownRef}
            className="py-[14px] px-4 gap-2 rounded-[4px] border-[#E4E4E7]"
            placeholder="사이즈를 선택해 주세요"
            options={getOptionsBySizeType()}/>
          </InputField>
        </div>
        <div className="bg-blue-50 w-full gap-10 p-4 text-blue-700 text-body2 leading-6 rounded-lg">
          <div className="flex gap-2">
            <img src={infoIcon} className="w-6 h-6"/>
            <p> 나에게 편한 신발 사이즈를 고려해서 추천사이즈를 알려드리기 위해 평소 신는 스니커즈 사이즈를 받고 있어요.</p>
          </div> 
        </div>
        <div className="h-[114px] mt-6 mx-5 mb-[34px]">
